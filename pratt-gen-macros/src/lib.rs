...
pub fn parse_impl_derive(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    match impl_parse(input.into()) {
        Ok(out) => out.into(),
        Err(err) => err.to_compile_error().into(),
    }
}

fn impl_parse(input: TokenStream) -> Result<TokenStream> {
    // Parse the input tokens into a syntax tree
    let ast = syn::parse2::<DeriveInput>(input).unwrap();
    // Only allow enum
    let Data::Enum(data) = ast.data else {
        Err(syn::Error::new_spanned(&ast.ident, "#[derive(ParseImpl)] can only handle enum. "))?
    };
    // Get the enum name
    let name = &ast.ident;
    // Verify there is exactly one generic parameter
    if ast.generics.lifetimes().into_iter().count() != 1 {
        Err(syn::Error::new_spanned(&ast.ident, "#[derive(ParseImpl)] need the enum to have exactly one lifetime argument. "))?
    }
    ...
    // Implement ParseImpl for enum
    Ok(quote! {
        impl<'a> ParseImpl<'a> for #name<'a> {
            fn parser_impl(
                source: Source<'a>, 
                arena: &'a Arena,
                precedence: u16,
